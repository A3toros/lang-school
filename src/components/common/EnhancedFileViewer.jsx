...
const EnhancedFileViewer = ({ file, isOpen, onClose, showAsInline = false }) => {
  ...
  const calculateOptimalScale = (data) => {
    ...
    // Container width (accounting for padding and borders)
    const containerWidth = 800 // Approximate available width
    
    // Calculate scale needed to fit content
    const requiredScale = containerWidth / estimatedWidth
    
    // Use horizontal scroll if scale would be too small (less than 0.6)
    if (requiredScale < 0.6) {
      setExcelScale(1.0) // No scaling, use horizontal scroll
      setExcelMaxHeight('85vh') // Give more height for horizontal scroll mode
    } else {
      setExcelScale(Math.max(0.6, Math.min(0.9, requiredScale))) // Scale between 0.6 and 0.9
      setExcelMaxHeight('70vh') // Normal height for scaled mode
    }
    ...
    return (
      <div className="h-full flex flex-col">
        <div className="p-4 border-b">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              title={isExpanded ? "Collapse view" : "Expand view (1.5x height)"}
            >
              {isExpanded ? (
                <>
        ...
        <div 
          ...
          style={{ 
            maxHeight: isExpanded ? '105vh' : '70vh',
            height: isExpanded ? '105vh' : '70vh'
          }}
        >
