...
async function createTeacher(event, user) {
  try {
    ...
    const { name, photo_url, description, username, password, meeting_id, meeting_password } = JSON.parse(event.body)
    ...
    try {
      ...
      const teacherResult = await client.query(
        'INSERT INTO teachers (name, photo_url, description, meeting_id, meeting_password) VALUES ($1, $2, $3, $4, $5) RETURNING *',
        [name, photo_url || null, description || null, meeting_id || null, meeting_password || null]
      )
...
async function updateTeacher(event, user) {
  try {
    const teacherId = parseInt(event.path.split('/')[3])
    const { name, photo_url, description, meeting_id, meeting_password } = JSON.parse(event.body)
    ...
    const queryText = `
      UPDATE teachers 
      SET name = $1, photo_url = $2, description = $3, meeting_id = $4, meeting_password = $5, updated_at = CURRENT_TIMESTAMP
      WHERE id = $6 AND is_active = true
      RETURNING *
    `
    
    const result = await query(queryText, [name, photo_url, description, meeting_id, meeting_password, teacherId])
