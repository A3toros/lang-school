-- Apply Dynamic lessons_per_week to Current Database
...
-- 3. Create function to update lessons_per_week automatically
...
BEGIN
  ...
  -- Update lessons_per_week for the student (count current week starting Monday)
  UPDATE students 
...
-- 5. Initialize lessons_per_week for all existing students based on current week
UPDATE students 
SET lessons_per_week = (
  SELECT COUNT(*) 
  FROM student_schedules 
...
COMMENT ON COLUMN students.lessons_per_week IS 'Dynamic counter of actual lessons per week (current week starting Monday). Auto-updated by database trigger when student_schedules change.';

-- 7. Verify the changes
SELECT 
  'Students with lessons_per_week = 0' as description,
  COUNT(*) as count
FROM students 
WHERE lessons_per_week = 0
UNION ALL
SELECT 
  'Students with lessons_per_week > 0' as description,
  COUNT(*) as count
